...
export class ContractERC721 extends Contract {
  ...
  async balanceOf(owner: string) {
    return this.call({method: 'balanceOf', args: [owner]}).then((res) => +res)
  }

  async ownerOf(tokenId: string) {
    return this.call({method: 'ownerOf', args: [tokenId]}).then((res) =>
  ...
  async transferFrom(params: {from: string; to: string; tokenId: string}) {
    ...
    return this.send({
      ...
      args: [from, to, tokenId, Web3.utils.stringToHex('')],
    })
  }
  ...
  async isApproved(params: {tokenId: string; operator: string}) {
    ...
    const addressApproved = await this.call({
      method: 'getApproved',
      args: [tokenId],
    })
    return (
      Web3.utils.toChecksumAddress(addressApproved) ===
