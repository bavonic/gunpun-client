...
import {chainBsc, chains} from './chains'
...
export function parseBlockchainError(data: {
  ...
  if (typeof error === 'object') {
    if (error.code === -32700)
      code = BlockchainErrorCode.INVALID_JSON_WAS_RECEIVED_BY_THE_SERVER
    else if (error.code === -32600)
      code = BlockchainErrorCode.INVALID_PAYLOAD_REQUEST
    else if (error.code === -32601)
      code = BlockchainErrorCode.METHOD_DOES_NOT_EXISTED_OR_NOT_AVAILABLE
    else if (error.code === -32602)
      code = BlockchainErrorCode.INVALID_METHOD_PARAMETERS
    else if (error.code === -32603)
      code = BlockchainErrorCode.INVALID_JSON_RPC_ERROR
    else if (error.code === -32000) code = BlockchainErrorCode.INVALID_INPUT
    else if (error.code === -32001)
      code = BlockchainErrorCode.RESOURCE_NOT_FOUND
    else if (error.code === -32002)
      code = BlockchainErrorCode.RESOURCE_UNAVAILABLE
    else if (error.code === -32003)
      code = BlockchainErrorCode.TRANSACTION_REJECTED
    else if (error.code === -32004)
      code = BlockchainErrorCode.METHOD_NOT_SUPPORTED
    else if (error.code === -32005)
      code = BlockchainErrorCode.REQUEST_LIMIT_EXCEEDED
    else if (error.code === 4001) code = BlockchainErrorCode.USER_REJECTED
    else if (error.code === 4100)
      code =
        BlockchainErrorCode.THE_REQUEST_ACCOUNT_OR_METHOD_HAS_NOT_BEEN_AUTHORIZED
    else if (error.code === 4200)
      code =
        BlockchainErrorCode.THE_REQUEST_METHOD_IS_NOT_SUPPORTED_BY_THIS_ETHEREUM_PROVIDER
    else if (error.code === 4900)
      code = BlockchainErrorCode.THE_PROVIDER_IS_DISCONNECTED_FROM_ALL_CHAINS
    else if (error.code === 4901)
      code =
        BlockchainErrorCode.THE_PROVIDER_IS_DISCONNECTED_FROM_THE_SPECIFIED_CHAIN
    else if (`${error.message}`.indexOf('not mined within 50 blocks') >= 0)
      code = BlockchainErrorCode.TRANSACTION_TIME_OUT
    else if (
      `${error.message}`.indexOf(
        'insufficient funds for gas * price + value'
      ) >= 0
    )
      code = BlockchainErrorCode.NOT_ENOUGH_BALANCE_FOR_GAS_FEE
    else if (
      `${error.message}`.indexOf('Transaction has been reverted by the EVM') >=
      0
    )
      code = BlockchainErrorCode.TRANSACTION_REVERTED_BY_THE_EVM
    else if (`${error.message}`.indexOf('Transaction underpriced') >= 0)
      code = BlockchainErrorCode.TRANSACTION_UNDER_PRICED
  ...
  try {
    ...
    const errorStringtify = errorStr
      .slice(errorStr.indexOf('{'), errorStr.lastIndexOf('}') + 1)
      .trim()
    if (errorStringtify) {
      ...
      if (errorStr.indexOf('execution reverted:') !== -1) {
        ...
        message = errorStr
          .slice(errorStr.indexOf(strCut), errorStr.length)
          .replace(strCut, '')
          .trim()
    ...
    try {
      if ((web3.currentProvider as any).isMetaMask) {
        ...
        const host =
          ...
          JSON.stringify(web3.currentProvider)
...
export function getAvailableWeb3(
  chainId: ChainId,
  ignoreRpcURL?: string
): Promise<{web3: Web3; rpc: string}> {
  const maxtLoop = 30
  let currentLoop = 0
  ...
  if (!rpcURLs.length)
    throw new BlockchainError({code: BlockchainErrorCode.CHAIN_NOT_SUPPORTED})
      ...
      await web3.eth
        ...
        .catch(() => {
          if (currentLoop >= maxtLoop)
            reject(
              new BlockchainError({
                code: BlockchainErrorCode.CANNOT_CONNECT_RPC_URL,
              })
            )
  ...
  return new Promise((resolve, reject) => {
    ...
    const action = async () => {
      if (!rpcURL) rpcURL = chain.rpcURLs[0]
        ...
        rpcURL = chain.rpcURLs.filter((v) => v !== rpcURL)[
          randomInt(0, chain.rpcURLs.length - 2)
        ]
      ...
      await web3.eth
        ...
        .catch((error) => {
          const e = parseBlockchainError({
            error,
            method: `web3.eth.getBalance(${wallet})`,
            type: 'READ',
            web3,
          })
...
export async function getTokenMetadata(tokenUri: string) {
  ...
  if (!tokenUri) {
    ...
    if (
      nftRespone &&
      typeof nftRespone.image === 'string' &&
      nftRespone.image.includes('ipfs')
    ) {
      nftRespone.image = nftRespone.image.replace(
        'ipfs://',
        'https://ipfs.io/ipfs/'
      )
    }
...
export async function getNameOfContract(chainId: ChainId, address: string) {
  ...
  return contract.call({method: 'name'}).catch(() => '')
}

export async function detectContractInterface(
  chainId: ChainId,
  address: string
) {
  try {
...
export function getAccount(privateKey: string): Account {
  const web3 = new Web3(chainBsc.rpcURLs[0])
...
export const validAddress = (wallet: string) => Web3.utils.isAddress(wallet)
