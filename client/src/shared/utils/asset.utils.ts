...
export const loadImage = (src: string): Promise<string> => {
  return new Promise((resolve) => {
    ...
    image.onerror = function () {
      resolve('');
      console.warn(`Load image error with src: ${src}`);
    }
...
export const loadSound = (src: string): Promise<HTMLAudioElement> => {
  return new Promise((resolve, reject) => {
    if (isMobile()) reject(new Error("Not Supported On Mobile"));
...
export let scriptLoaded: string[] = [];
export const loadScript = function (src: string) {
  return new Promise((resolve, reject) => {
    if (scriptLoaded.includes(src)) resolve(src);
    scriptLoaded.push(src);
